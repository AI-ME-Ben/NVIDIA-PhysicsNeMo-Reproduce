...
class solidCustomValidatorPlotter(ValidatorPlotter):
    ...
    def __call__(self, invar, true_outvar, pred_outvar):
        ...
        # Interpolate 'c' values
        c_true, c_pred = solidCustomValidatorPlotter.interpolate_output(
            y, z, [c_true, c_pred], extent
        )
    ...
    def interpolate_output(y, z, values, extent):
        ...
        mask = fluidCustomValidatorPlotter.heat_sink_mask(yi, zi)
        inverted_mask = ~mask  # Invert the mask to mask fluid regions
        ...
        for i in range(len(interpolated_values)):
            interpolated_values[i][inverted_mask] = np.nan
