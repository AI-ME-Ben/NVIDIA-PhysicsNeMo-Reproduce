...
# Define base class for common functionality
class BaseValidatorPlotter(ValidatorPlotter):

    @staticmethod
    def heat_sink_mask_yz(yi, zi):
    ...
    def plot(self, x, y, c_true, c_pred, extent, colorbar_limits, titles, mask_func, mask_inverted=False):
        ...
        for i, ax in enumerate(axes):
            ...
            ax.set_xlabel("z" if mask_func == self.heat_sink_mask_yz else "x")
            ax.set_ylabel("y")
...
# Define custom class for fluid
class fluidCustomValidatorPlotter(BaseValidatorPlotter):

    def plot_yz(self, invar, true_outvar, pred_outvar):
        ...
        return self.plot(y, z, c_true, c_pred, extent, colorbar_limits, titles, self.heat_sink_mask_yz, mask_inverted=False)

    def plot_xy(self, invar, true_outvar, pred_outvar):
        "Custom plotting function for validator (showing 'c' in x-y plane)"

        # Filter data for z = 0
        mask = np.isclose(invar["z"][:, 0], 0)
        x, y = invar["x"][mask, 0], invar["y"][mask, 0]
        c_true = true_outvar["theta_f"][mask, 0] * 273.15
        c_pred = pred_outvar["theta_f"][mask, 0] * 273.15

        # Define fixed extent
        extent = (-2.5, 2.5, -0.5, 0.5)
        ...
        return self.plot(x, y, c_true, c_pred, extent, colorbar_limits, titles, self.heat_sink_mask_xy, mask_inverted=False)

# Define custom class for solid
class solidCustomValidatorPlotter(BaseValidatorPlotter):
    ...
    def __call__(self, invar, true_outvar, pred_outvar):
        ...
        return self.plot(y, z, c_true, c_pred, extent, colorbar_limits, titles, self.heat_sink_mask_yz, mask_inverted=True)
